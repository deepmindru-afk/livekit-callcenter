...
(function() {
    ...
    async function connectToRoom(token) {
        ...
        try {
            ...
            if (error.message.includes('Failed to fetch') || 
                ...
                console.error('Network error detected. Please check your LiveKit server URL and ensure it is accessible.');
            }
    ...
    async function createSIPInvite(phoneNumber, roomName) {
        ...
        try {
            ...
            const participantIdentity = localParticipant ? localParticipant.identity : `caller_${Date.now()}`;
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to create SIP participant: ${errorData.detail || response.statusText}`);
            }
    ...
    async function publishLocalAudio() {
        ...
        try {
            ...
            try {
                ...
                localTrackPublication = await localParticipant.publishTrack(microphoneTrack);
                ...
                if (LivekitClient.createDummyAudioTrack) {
                    try {
                        ...
                        localTrackPublication = await localParticipant.publishTrack(silentTrack);
    ...
    function setupAudioLevelMonitoring(audioTrack) {
        ...
        try {
            ...
            const interval = setInterval(() => {
                if (!audioTrack.isEnabled || !audioTrack.mediaStreamTrack || audioTrack.mediaStreamTrack.readyState !== 'live') {
                ...
                if (average < 5) {
                    ...
                    if (silenceCounter >= silenceThreshold) {
                        console.warn(`Microphone might not be capturing audio - ${silenceCounter} consecutive silent samples detected`);
                        
                        // Show a notification to the user after extended silence
                        if (silenceCounter === silenceThreshold || silenceCounter % 10 === 0) {
                            // Optional: Notify user about potential microphone issue
                            const event = new CustomEvent('audioIssueDetected', {
                                detail: { type: 'silence', message: 'Microphone not detecting audio' }
                            });
    ...
    function setupRoomEventListeners() {
        ...
        room.on(LivekitClient.RoomEvent.ParticipantDisconnected, (participant) => {
            ...
            if (participant.identity.startsWith('pstn_') || participant.identity.startsWith('caller_')) {
                callHandler.endCall();
                
                // Dispatch event for main.js to handle UI updates
                window.dispatchEvent(new CustomEvent('callEnded', {
                    detail: { reason: 'remoteHangup' }
                }));
            }
        });
        ...
        room.on(LivekitClient.RoomEvent.TrackPublished, (publication, participant) => {
            console.log('Track published by participant:', participant.identity, 'track kind:', publication.kind);
            
            // Automatically subscribe to track if it's not already subscribed
            if (!publication.isSubscribed) {
                console.log(`Subscribing to ${publication.kind} track from ${participant.identity}`);
        ...
        room.on(LivekitClient.RoomEvent.TrackSubscribed, (track, publication, participant) => {
            console.log('Subscribed to track:', track.kind, 'from participant:', participant.identity);
            ...
            if (track.kind === 'audio') {
                ...
                try {
                    ...
                    remoteAudioElement.play()
                        ...
                        .catch(error => {
                            ...
                            remoteAudioElement.play()
                                .then(() => console.log('Remote audio playback started after user interaction'))
                                .catch(e => console.error('Still failed to play audio after user interaction:', e));
                        });
                ...
                track.on('audioLevelChanged', (level) => {
                    if (level > 0.01) { // Only log when there's meaningful audio
        ...
        room.on(LivekitClient.RoomEvent.TrackUnsubscribed, (track, publication, participant) => {
            console.log('Unsubscribed from track:', track.kind, 'from participant:', participant.identity);
