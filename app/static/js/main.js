...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log("Agent from localStorage:", agent);
  ...
  if (!agent.id) {
    console.error("Agent ID is missing! Agent object:", agent);
    alert("Agent ID is missing. Please log in again.");
  ...
  // Listen for call ended events from LiveKit
  window.addEventListener("callEnded", (event) => {
    console.log("🔥 Call ended event received from LiveKit:", event.detail);
    console.log(
      "🔥 Current activeCall state:",
      activeCall || window.activeCall
    );
    handleCallEnded(event.detail);
  });
  ...
  function connectWebSocket() {
    ...
    // Make socket available globally for auto-assignment manager
    window.socket = socket;

    socket.onopen = () => {
      console.log("✅ WebSocket connection established successfully");
      console.log("Agent ID connected:", agent.id);

      // Update debug panel if available
      const debugWebSocket = document.getElementById("debugWebSocket");
      if (debugWebSocket) {
        debugWebSocket.textContent = "Connected";
      }
    };
    ...
    socket.onclose = (event) => {
      ...
      const debugWebSocket = document.getElementById("debugWebSocket");
      if (debugWebSocket) {
        debugWebSocket.textContent = "Disconnected";
      }
    ...
    socket.onerror = (error) => {
      ...
      const debugWebSocket = document.getElementById("debugWebSocket");
      if (debugWebSocket) {
        debugWebSocket.textContent = "Error";
      }
    };
  }

  function handleWebSocketMessage(data) {
    // First, let auto-assignment manager handle its messages
    if (window.autoAssignmentManager) {
      window.autoAssignmentManager.handleWebSocketMessage(data);
    }
    ...
    switch (data.type) {
      ...
      case "room_update":
        ...
        const inboundTabActive = document
          .querySelector("#inbound-tab")
      ...
      case "call_invitation":
        // Handled by auto-assignment manager
  ...
  async function handleOutboundCall() {
    ...
    try {
      ...
      if (callHandler) {
        ...
        try {
          await callHandler.startOutboundCall(
            callData.livekit_token,
            phoneNumber,
            roomName
          );
        } catch (callError) {
          ...
          alert(
            `Call connection error: ${callError.message}. Call record created but connection failed.`
          );
  ...
  async function handleHangup() {
    // Check both local and global activeCall
    const currentCall = activeCall || window.activeCall;

    if (!currentCall) {
      console.error("No active call to hang up");
      console.log("Local activeCall:", activeCall);
      console.log("Global window.activeCall:", window.activeCall);
      alert("No active call to hang up");
      return;
    }

    // Sync the local variable with global if needed
    if (!activeCall && window.activeCall) {
      activeCall = window.activeCall;
      console.log("Synced local activeCall with global");
    }

    try {
      console.log("Ending call:", activeCall);

      // First, disconnect from LiveKit room gracefully
      if (callHandler) {
        try {
          ...
          console.error("Error disconnecting from LiveKit:", liveKitError);
          // Continue with server cleanup even if LiveKit disconnect fails
        }
      }

      // Then notify the server to end the call
      if (activeCall.direction === "inbound" && activeCall.room_name) {
        console.log("Ending inbound room call...");
        ...
        console.log("Ending regular call...");
      ...
      console.log("Server call end request completed");

      // Finally, clean up the UI
      handleCallEnded();
    } catch (error) {
      console.error("Hangup error:", error);
      // Still clean up UI even if server request fails
      handleCallEnded();
  ...
  function handleCallEnded(data = {}) {
    console.log("Cleaning up call UI and state...");
    ...
    // Reset active call
    const wasInCall = !!(activeCall || window.activeCall);
    activeCall = null;
    ...
    // Clear any call-related UI state
    callerId.textContent = "";

    // Only update status to Available if we were actually in a call
    if (wasInCall) {
      console.log("Call ended, updating agent status to Available...");
      // Small delay to ensure LiveKit cleanup is complete
      setTimeout(() => {
        updateAgentStatus("Available")
          .then(() => {
          ...
          .catch((error) => {
            ...
            setTimeout(() => {
              ...
              updateAgentStatus("Available").catch((retryError) => {
  ...
  // Make functions available globally for auto-assignment
  ...
  // Make activeCall accessible globally
  window.getActiveCall = () => activeCall;
  window.setActiveCall = (call) => {
    activeCall = call;
    ...
    try {
      ...
      data.forEach((call) => {
        ...
        const formattedDate = new Date(
          call.timestamp || call.start_time
        ).toLocaleString();
        ...
        row.innerHTML = `
                    ...
                    <td>${
                      call.caller_id || call.to_number || call.from_number
                    }</td>
                    <td>${duration}</td>
                    <td>${call.status}</td>
                `;
  ...
  async function loadInboundCalls() {
    try {
      ...
      const newRooms = rooms
        .filter((room) => !previousRooms.includes(room.room_name))
      ...
      const enhancedRooms = await Promise.all(
        rooms.map(async (room) => {
          try {
            ...
            console.warn(
              `Could not fetch details for room ${room.room_name}:`,
              err
            );
          }
      ...
      enhancedRooms.forEach((room) => {
        ...
        row.innerHTML = `
                    ...
                    <td><button class="join-call-btn" data-room="${
                      room.room_name
                    }">Join Call</button></td>
                `;
